...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const { data, error, isLoading } = useSWR<PersonalInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/personals` : null),
    () => getPersonals({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Personal',
      path: isTenantUser ? `/personals/view/${data?.[0]?.id}` : '/personals',
      entity: 'personal',
      service: AccessServiceEnum.PROJECT,
      icon: FiInfo,
    },
    { name: 'Budget', path: '/budgets', entity: 'budget', service: AccessServiceEnum.PROJECT, icon: FiDollarSign },
    { name: 'Expense', path: '/expenses', entity: 'expense', service: AccessServiceEnum.PROJECT, icon: FiShoppingCart },
