...
import { getPersonalById, updatePersonalById } from 'apiSdk/personals';
...
function PersonalEditPage() {
  ...
  const handleSubmit = async (values: PersonalInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updatePersonalById(id, values);
      mutate(updated);
      resetForm();
  ...
  const formik = useFormik<PersonalInterface>({
    initialValues: data,
